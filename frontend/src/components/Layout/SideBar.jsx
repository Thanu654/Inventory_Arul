...
const SideBar = () => {
  ...
  const menuItems = [
    ...
    { id: 'delivery', label: 'Delivery', icon: DeliveryIcon, path: '/delivery' },
    { id: 'transactions', label: 'Transactions', icon: TransactionsIcon, path: '/transactions' },
  ...
  return (
    <div className={`h-screen flex flex-col bg-gradient-to-b from-gray-900 to-gray-950 text-gray-200 transition-all duration-300 ${isCollapsed ? 'w-16' : 'w-64'} shadow-xl relative`}>
      ...
      {!isCollapsed && (
        <div className="lg:hidden fixed inset-0 bg-black bg-opacity-50 z-10" onClick={() => setIsCollapsed(true)}></div>
      )}
    </div>
  );
};
