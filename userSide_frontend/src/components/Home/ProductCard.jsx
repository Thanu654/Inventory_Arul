// Updated ProductCard.jsx - Mobile Optimized with Blue Theme
...
const ProductCard = ({ product, category }) => {
  ...
  return (
    <div 
      className="bg-white rounded-xl md:rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 hover:-translate-y-1 relative group border border-blue-50"
    ...
    >
      ...
      {product.price > 50000 && (
        <div className="absolute top-2 left-2 z-10">
          <div className="px-2 py-1 bg-gradient-to-r from-amber-500 to-yellow-500 text-white text-xs font-bold rounded-lg shadow-lg">
            PREMIUM
          </div>
        </div>
      )}
      ...
      {/* Image Container */}
      <div className="relative h-36 sm:h-40 md:h-44 overflow-hidden bg-gradient-to-br from-blue-50 to-cyan-50">
        {!imageLoaded && (
          <div className="absolute inset-0 bg-gradient-to-r from-blue-100/50 via-blue-50/50 to-blue-100/50 animate-shimmer bg-[length:200%_100%]"></div>
        )}
      ...
      <div className="p-3 md:p-4">
        {/* Category tag */}
        <div className="mb-1">
          <span className="text-blue-600 text-xs font-bold uppercase tracking-wide bg-blue-50 px-2 py-1 rounded-full">
            {category}
          </span>
        </div>
        ...
        <div className="flex justify-between items-center my-1 md:my-2 gap-2">
          <h3 className="text-gray-800 font-bold text-sm md:text-base flex-1 line-clamp-1 group-hover:text-blue-600 transition-colors duration-300">
            {product.name}
          </h3>
          <div 
            className={`px-2 py-1 rounded-lg text-xs font-semibold whitespace-nowrap transform transition-all duration-300 ${
              product.quantity > 0 
                ? 'bg-green-50 text-green-600 group-hover:scale-105' 
        ...
        <div className="mb-2 md:mb-3">
          <p className="text-gray-600 text-xs md:text-sm leading-relaxed line-clamp-2 group-hover:text-gray-700 transition-colors duration-300">
            {product.description?.substring(0, 60)}...
          </p>
        </div>

        {/* Price and WhatsApp button */}
        <div className="flex justify-between items-center mt-1 md:mt-2 gap-2">
          <div className="flex-1 space-y-0.5">
            <span className="text-gray-800 font-extrabold text-base md:text-lg block transform group-hover:scale-105 transition-transform duration-300">
          ...
          <button 
            className="relative bg-gradient-to-br from-green-500 to-green-600 text-white w-8 h-8 md:w-9 md:h-9 rounded-full flex items-center justify-center cursor-pointer transition-all hover:scale-110 hover:shadow-md shadow-sm hover:shadow-green-300/50 overflow-hidden group/button"
